    ...
    if args[1] in connection.users["users"].keys() :
        ...
        if len(args) == 4 :
            if int(args[2]) == 0 :
                ...
                connection.msg(info["channel"], str(len(fnmatch.filter([connection.whois[x] for x in connection.channellist[info["channel"]]], args[3]))) + " users ignored with this mask")
