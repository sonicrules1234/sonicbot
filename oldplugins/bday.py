...
def main(connection, info, args) :
    ...
    if error2 == 1 :
        ...
        if bday[1] == now[1] and bday[2] == now[2] :
            ...
            secstillbday = int(time.mktime(nextbday)) - int(time.time())
            days = int(secstillbday / (60 * 60 * 24))
        if days != 0 : connection.msg(info["channel"], _("You are %(numyears)s years old and have %(numdays)s days until your next birthday.") % dict(numyears=str(years), numdays=str(days)))
        else : connection.msg(info["channel"], _("Today is your birthday.  You are %(numyears)s years old") % dict(numyears=str(years)))
    elif error == 0 :
        bday = time.strptime(birthd, "%m/%d/%Y")
        birthday = time.strftime("%B %d %y", bday)
        now = time.gmtime()
