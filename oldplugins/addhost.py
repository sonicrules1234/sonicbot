...
def main(connection, info, args) :
    ...
    if info["sender"] in connection.users["users"].keys() :
        if hashlib.sha512(args[1]).hexdigest() == connection.users["users"][info["sender"]]["password"] and connection.hostnames[info["sender"]] not in connection.users["users"][info["sender"]]["hostname"] :
