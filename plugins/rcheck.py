...
def main(connection, info, args, world) :
    ...
    connection.ircsend(info["channel"], "%s reported %s in %s with the reason '%s' and a priority level of %s on %s" % (reports["reports"][int(args[1])]["reporter"][0], reports["reports"][int(args[1])]["accused"], reports["reports"][int(args[1])]["channel"], reports["reports"][int(args[1])]["reason"], reports["reports"][int(args[1])]["priority"], time.strftime("%b %d %Y, %H:%M:%S %Z", time.gmtime(reports["reports"][int(args[1])]["time"]))))
    reports.close()
