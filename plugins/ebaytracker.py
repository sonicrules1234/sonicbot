    ...
    if feedurl.startswith("http://cgi.ebay.com/") :
        if onoff == "on" :
            ...
            if feedurl in world.feeds[connection.host][info["channel"]].keys() :
                    ...
                    connection.ircsend(info["channel"], _("Stopped tracking item at %(url)s") % dict(url=feedurl))
            else : connection.ircsend(info["channel"], _("No such item being tracked."))
...
def get_feed(connection, info, args, feedurl, world, indexnum, title) :
    ...
    while world.feeds[connection.host][info["channel"]][feedurl][indexnum] :
        ...
        if price != feeds[feedurl]["updated"] and feeds[feedurl]["updated"] != 0 :
            ...
            feeds.sync()
            connection.ircsend(info["channel"], _("Another bid on %(itemtitle)s has been made!  The price is now %(newprice)s") % dict(itemtitle=title, newprice=price))
        elif feeds[feedurl]["updated"] == 0 :
            feeds[feedurl]["updated"] = price
            feeds.sync()
            connection.ircsend(info["channel"], _("Started tracking '%(itemtitle)s' with price of %(newprice)s.") % dict(itemtitle=title, newprice=price))
        feeds.close()
        time.sleep(120)
