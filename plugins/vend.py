...
def main(connection, info, args, world) :
    ...
    if votes["networks"][world.hostnicks[connection.host]][int(args[1]) - 1]["started"] :
        ...
        connection.ircsend(info["channel"], "\n".join(["Choice %s: '%s' got %s votes" % (str(x + 1), votes["networks"][world.hostnicks[connection.host]][int(args[1]) - 1]["choices"][x]["choice"], str(len(votes["networks"][world.hostnicks[connection.host]][int(args[1]) - 1]["choices"][x]["voters"]))) for x in range(len(votes["networks"][world.hostnicks[connection.host]][int(args[1]) - 1]["choices"]))]))
        scores = [len(votes["networks"][world.hostnicks[connection.host]][int(args[1]) - 1]["choices"][x]["voters"]) for x in range(len(votes["networks"][world.hostnicks[connection.host]][int(args[1]) - 1]["choices"]))]
